...
# to be at least 0.2 for them to be candidates for merging into a line

LINE_OVERLAP_RATIO = 0.2
...
MAX_LINE_WIDTH = 10             # Close together parallel edges a polygons are converted to lines
...
class LineFinder:
    ...
    #========================================================
        ...
        for (line0, line1) in itertools.combinations(boundary_line_coords, 2):
            ...
            l0 = Line.from_coords(line0)            # type: ignore
            l1 = Line.from_coords(line1)            # type: ignore
            if l0.parallel(l1):
                ...
                if ((pt := p0.mid_point(p1)) is not None
                 ...
                 and (distance := p0.separation(p1)) <= self.__max_line_width):
                    ...
                    if (#p0.overlap(p1, True) > MIN_LINE_ASPECT_RATIO*distance and
                        p0.overlap(p1, False)/p0.overlap(p1, True) >= LINE_OVERLAP_RATIO
                      ):
        ...
        if len(ends_graph):
            for end_nodes in nx.connected_components(ends_graph):
                if len(end_nodes) == 4:     # We most likely have an arrow head
                    ...
                    add_class(shape.element, 'error')
                    logging.warning(f'Bad arrow shape? {shape.id}, nodes={len(end_nodes)}')
